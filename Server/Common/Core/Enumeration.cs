...
public abstract class Enumeration : IComparable
{
    ...
    protected static T FromName<T>(string name) where T : Enumeration
        => Parse<T, string>(name, nameof(name), item => item.Name == name);

    protected static string NameFromValue<T>(int value) where T : Enumeration
...
public sealed class ConcreteEnumeration : Enumeration
{
    public ConcreteEnumeration(int value, string name)
        : base(value, name)
    {
    }
} 